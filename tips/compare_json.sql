ALTER FUNCTION [dbo].[Compare_JsonObject] (
...
BEGIN
	IF (
			IsJson(@SourceJSON) = 1
			AND IsJson(@TargetJSON) = 1
			) --don't try anything if either json is invalid
	BEGIN
		...
		SELECT
			...
			CASE 
				WHEN The_Source.TheValue = The_Target.TheValue
					THEN '=='
				ELSE CASE 
						WHEN The_Source.ThePath IS NULL
							THEN '-'
						ELSE '<'
						END + CASE 
						WHEN The_Target.ThePath IS NULL
							THEN '-'
						ELSE '>'
						END
